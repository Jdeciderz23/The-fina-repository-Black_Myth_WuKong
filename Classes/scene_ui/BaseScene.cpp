...
void BaseScene::initEnemy()
{
    ...
    auto enemyDeathListener = cocos2d::EventListenerCustom::create("enemy_died", [this](cocos2d::EventCustom* event) {
        CCLOG("BaseScene: 接收到敌人死亡事件");
        Enemy* deadEnemy = static_cast<Enemy*>(event->getUserData());
        if (deadEnemy) {
            CCLOG("BaseScene: 处理敌人 %p 的死亡", (void*)deadEnemy);
...
void BaseScene::removeDeadEnemy(Enemy* deadEnemy) {
    if (!deadEnemy) {
        CCLOG("BaseScene::removeDeadEnemy: 无效的敌人指针");
        return;
    }
    
    CCLOG("BaseScene::removeDeadEnemy: 移除敌人 %p", (void*)deadEnemy);
    
    // 从敌人列表中移除死亡敌人
    auto it = std::find(_enemies.begin(), _enemies.end(), deadEnemy);
    if (it != _enemies.end()) {
        _enemies.erase(it);
        CCLOG("BaseScene::removeDeadEnemy: 敌人已从列表中移除，剩余 %zu 个敌人", _enemies.size());
    } else {
        CCLOG("BaseScene::removeDeadEnemy: 敌人未在列表中找到");
    }
}